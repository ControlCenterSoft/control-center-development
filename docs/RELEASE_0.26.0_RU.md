...
Кандидат содержит проверки permission boundary, first-login boundary, отсутствия утечки содержимого Audit и внутренних ошибок verifier, корректного пустого состояния, а также fail-closed поведения при невозможности сохранить success-evidence. Для PostgreSQL предусмотрена отдельная интеграционная проверка чтения и верификации сохранённого префикса и отмены операции через context.

## Подготовка к qualification

В кандидат включены контракт OpenAPI, in-memory и PostgreSQL implementations, HTTP API, negative-path и persistence-oriented tests и документация. До официального выпуска обязательны отдельные deterministic qualification/release gates на замороженном exact SHA.
