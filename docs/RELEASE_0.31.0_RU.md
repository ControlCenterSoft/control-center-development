...
- exact-revision operational workflow evidence binding `approval → terminal Job result → recovery`, включая false-success guard и strict storage/transport validator — отдельный NR2 slice.

Наличие этих веток не меняет canonical `main` и не является release evidence.
...
2. Qualification/integration exact-revision operational workflow evidence binding.
3. Полный `approval → Job → verification → recovery` E2E, включая negative/failure/false-success cases.
