...
Контракт сознательно **не** добавляет управление пользователями/ролями, не создаёт новый actor selector и не выдаёт authorization/execution authority. `self_only=true`, `mutation_authorized=false` являются инвариантами ответа.
...
- Порядок sessions/grants/permissions детерминирован, чтобы UI не зависел от порядка backend storage.
...
4. Добавить exact-head API/contract tests, schema validation, accessibility/UI rendering tests и permission-denied explanations.
