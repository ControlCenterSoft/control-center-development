...
SOURCE_ASSERTIONS = {
    ...
    "internal/incidents/httpapi/web_v1.go": (
        "read-only",
        "не предоставляет элементов управления изменением состояния",
        "Cache-Control",
        "no-store",
    ),
}
...
def main() -> int:
    ...
    if not version_path.is_file() or version_path.read_text(encoding="utf-8").strip() != CANDIDATE_VERSION:
        raise ValueError("source VERSION must be 0.32.1 for scope qualification")
    ...
    incident_template = checked_text(root / "internal/incidents/httpapi/web_v1.go")
    if "acknowledge/resolve/mutation authority" in incident_template:
        raise ValueError("Incident browser still embeds mutation-route wording in rendered read-only evidence")
