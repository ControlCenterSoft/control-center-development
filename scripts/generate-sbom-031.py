...
def main() -> None:
    if len(sys.argv) != 4:
        fail("usage: generate-sbom-031.py MANIFEST OUTPUT CANDIDATE_SHA")
    ...
    if manifest.get("product_version") != "0.31.0":
        fail("third-party manifest is not bound to Control Center 0.31.0")
    ...
    root_ref = f"pkg:generic/control-center@0.31.0?revision={candidate_sha}"
