...
python3 - "$qualification" "$candidate_sha" "$public_ci_run_id" "$binary_digest" "$source_digest" "$sbom_digest" "$notices_digest" "$stable_sha256" <<'PY'
import json,sys
path,sha,run_id,binary_digest,source_digest,sbom_digest,notices_digest,stable_digest=sys.argv[1:]
data={
...
python3 - "$provenance" "$candidate_sha" "$public_ci_run_id" "$binary_digest" "$source_digest" "$sbom_digest" "$notices_digest" <<'PY'
import json,sys
...
echo "CC_032_EXACT_SHA_QUALIFICATION=PASS candidate_sha=$candidate_sha public_ci_run_id=$public_ci_run_id stable_base=$stable_version"
