...
Статус: **нормативная продуктовая архитектура**.

Этот документ описывает целевую архитектуру продукта. Если текущая опубликованная версия ещё не поддерживает описанную возможность, такая возможность считается целевой или предварительной и не должна представляться как уже доступная пользователю. Архитектурное развитие должно сохранять совместимость общих контрактов и не создавать параллельные несовместимые модели состояния, полномочий или выполнения.
...
## 2. Границы архитектурного контракта

Документ фиксирует доменные границы, обязательные инварианты и целевые возможности Control Center. Наличие архитектурного описания само по себе не означает пользовательскую доступность функции: реализованной и поддерживаемой считается только возможность, вошедшая в официально опубликованный релиз и имеющая соответствующий проверяемый lifecycle, failure/recovery и эксплуатационный контракт.

Развитие продукта должно сохранять единые Identity/RBAC, Desired/Actual State, Changes/Jobs, Audit, Network, Capacity, Lifecycle и Recovery contracts. Новая подсистема не должна создавать отдельную несовместимую модель полномочий, состояния или привилегированного выполнения.

Предварительные возможности и будущие HA/DR/Market-профили должны быть явно отделены от текущего опубликованного baseline. Пользовательская и эксплуатационная документация обязана описывать фактические границы установленной версии и не выдавать целевую архитектуру за уже сертифицированное production-поведение.
