...
На 10.09.2026 последний опубликованный исходный релиз — **0.15.0**. Отдельный стабильный бинарный канал распространения подтверждён до **0.3.1**.
...
Критически важно: наличие multi-node объектов или HA-контрактов не означает, что production failover уже поддерживается. HA считается поддержанным только для конкретных ролей и версий, для которых опубликованы и проверены quorum/fencing/failover/recovery процедуры.
...
Опубликованная линия 0.7–0.15 развивает этот контур как **advisory-only**. В 0.15 опубликована bounded nonlinear Workload Curve: piecewise interpolation разрешена только внутри измеренного диапазона, extrapolation за пределы evidence запрещена.
...
- backup integrity и retention;
- restore verification;
- PostgreSQL backup/PITR для поддерживаемых профилей;
- object-level recovery там, где это безопасно;
- RPO/RTO policies;
- регулярные restore drills;
- восстановление после отказа узла и после ошибочных действий оператора/приложения.
...
Последний опубликованный исходный релиз 0.15.0 подтверждает развитие Capacity Intelligence до bounded nonlinear Workload Curve. Более новые capability stages должны считаться предварительными до официальной публикации соответствующих версий.
