...
После базового `0.7.0` forecast/what-if контура подготовлены дополнительные advisory-only кандидаты. Они не меняют опубликованный baseline `0.6.0` и не получают права автоматически изменять инфраструктуру:

- `0.8.0` — Placement Advice: детерминированный рекомендательный выбор уже подходящего узла для дополнительной нагрузки с учётом safe capacity, bottleneck reserve, confidence и общего failure reserve парка;
- `0.9.0` — Bottleneck Report: отдельная read-only классификация `critical` / `warning` / `unknown` / `healthy`, сопоставляющая общий capacity reserve с резервом ограничивающей метрики;
- `0.10.0` — Capacity Horizon: прогноз `days_to_safe_capacity`, связывающий workload forecast с безопасной fleet capacity после учёта failure reserve;
...
Все перечисленные кандидаты остаются аналитическими: `advisory_only = true`, не изменяют Desired State или Actual State, не выполняют placement/drain/migration/resize как действие и не запускают provider-команды. Для кандидатных номеров без подтверждённого release-описания отдельный пользовательский состав в этой дорожной карте не фиксируется.
