...
Статус на 10.09.2026: последний опубликованный исходный релиз — **0.9.0**. Отдельный стабильный бинарный канал опубликован до **0.3.1**. Версии выше `0.9.0` считаются предварительными до отдельной официальной публикации.
...
Опубликован детерминированный отчёт по узким местам парка. Он сопоставляет safe-capacity reserve и фактически ограничивающую метрику, классифицирует состояние и формирует только рекомендательное действие. `0.9.0` не выполняет placement, resize, drain, migration или автоматическое изменение инфраструктуры.

## Кандидатная Capacity-линия после 0.9.0

Следующие версии существуют как предварительные кандидаты и не являются опубликованными возможностями:

- `0.10.0` — Capacity Horizon: прогноз времени до исчерпания безопасной ёмкости;
