...
Статус на 10.09.2026: последний подтверждённый официальный исходный релиз — **0.22.0**. Отдельный стабильный бинарный канал остаётся на **0.3.1**. Эти линии имеют разные release identity и не должны смешиваться.
...
- **0.22.0** — Session Security Policy: absolute TTL, idle timeout, инвентаризация и отзыв собственных сессий с durable persistence и обязательным Audit.

Capacity-функции опубликованной линии остаются advisory-only и не являются разрешением на автоматическое изменение инфраструктуры.
...
**Authentication**: после чистой установки используется начальная локальная учётная запись `admin/admin` с обязательной сменой пароля при первом входе; обновление не сбрасывает уже установленный пароль. В 0.22.0 опубликована дополнительная session-security policy с абсолютным TTL и idle timeout; активность сессии не продлевает её абсолютный срок жизни.
