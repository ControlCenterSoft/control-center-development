...
Последний официально опубликованный исходный релиз Control Center — **0.24.0**. Отдельный стабильный бинарный канал подтверждён до **0.3.1**. Эти каналы нельзя смешивать: функция считается доступной в конкретном канале только после фактической публикации соответствующего релиза этого канала.
...
Опубликованная исходная линия до 0.24.0 включает:
...
Для опубликованной исходной линии **0.6.0–0.24.0** после чистой установки создаётся локальный пользователь `admin` с первоначальным паролем `admin`. Первый вход обязан привести к смене пароля; до смены обычная работа запрещена. При обновлении пользовательский пароль сохраняется и не сбрасывается к первоначальному значению.

Отдельный stable-binary выпуск **0.3.1** относится к более ранней bootstrap-модели аутентификации. Правила `admin/admin` более новой исходной линии нельзя автоматически применять к stable 0.3.1; для него используется документация и bootstrap credential его собственного канала.
