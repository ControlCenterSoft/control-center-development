...
ORDER BY r.sequence,i.key`)
	...
	for rows.Next() {
		...
		var content string
		if err := rows.Scan(&revision.ID, &revision.Sequence, &revision.Digest, &content, &revision.CreatedAt, &revision.CreatedBy, &revision.IdempotencyKey, &revision.Fingerprint); err != nil { rows.Close(); return state, err }
		revision.Content = json.RawMessage(content)
		state.Revisions = append(state.Revisions, revision)
	}
	if err := rows.Close(); err != nil { return state, err }

	rows, err = s.db.QueryContext(ctx, changeSelect+` ORDER BY c.created_at,c.id`)
	if err != nil { return state, err }
	for rows.Next() {
		persisted, err := scanPersistedChange(rows)
		if err != nil { rows.Close(); return state, err }
...
func (s *OrchestrationState) CreateRevision(ctx context.Context, createdBy, key, fingerprint string, content json.RawMessage, now time.Time) (orchestrationapi.PersistedRevision, error) {
	if createdBy == "" || key == "" || fingerprint == "" || !json.Valid(content) || now.IsZero() { return orchestrationapi.PersistedRevision{}, errors.New("complete revision persistence request is required") }
	tx, err := s.db.BeginTx(ctx, nil); if err != nil { return orchestrationapi.PersistedRevision{}, err }; defer tx.Rollback()
	if _, err := tx.ExecContext(ctx, `SELECT pg_advisory_xact_lock(hashtext($1))`, "revision:"+key); err != nil { return orchestrationapi.PersistedRevision{}, err }
	if existing, storedFingerprint, found, err := revisionByIdempotency(ctx, tx, key); err != nil { return orchestrationapi.PersistedRevision{}, err } else if found {
		if storedFingerprint != fingerprint { return orchestrationapi.PersistedRevision{}, job.ErrIdempotencyConflict }
		return existing, tx.Commit()
	}
	digest := "sha256:" + fingerprint
	if existing, found, err := revisionByDigest(ctx, tx, digest); err != nil { return orchestrationapi.PersistedRevision{}, err } else if found {
		if _, err := tx.ExecContext(ctx, `INSERT INTO cc_idempotency_keys(scope,key,fingerprint,resource_id,created_at) VALUES ('revision',$1,$2,$3,$4)`, key, fingerprint, existing.ID, now.UTC()); err != nil { return orchestrationapi.PersistedRevision{}, err }
		existing.IdempotencyKey, existing.Fingerprint = key, fingerprint
		return existing, tx.Commit()
	}
	if _, err := tx.ExecContext(ctx, `LOCK TABLE cc_config_revisions IN EXCLUSIVE MODE`); err != nil { return orchestrationapi.PersistedRevision{}, err }
	var sequence uint64
	if err := tx.QueryRowContext(ctx, `SELECT COALESCE(MAX(sequence),0)+1 FROM cc_config_revisions`).Scan(&sequence); err != nil { return orchestrationapi.PersistedRevision{}, err }
	...
	if err != nil { return orchestrationapi.PersistedRevision{}, err }
	...
	if err != nil { return orchestrationapi.PersistedRevision{}, err }
	if err := tx.Commit(); err != nil { return orchestrationapi.PersistedRevision{}, err }
	return revision, nil
}

func revisionByIdempotency(ctx context.Context, tx *sql.Tx, key string) (orchestrationapi.PersistedRevision, string, bool, error) {
	var revision orchestrationapi.PersistedRevision; var content, fingerprint string
	...
	if errors.Is(err, sql.ErrNoRows) { return revision, "", false, nil }
	if err != nil { return revision, "", false, err }
...
func revisionByDigest(ctx context.Context, tx *sql.Tx, digest string) (orchestrationapi.PersistedRevision, bool, error) {
	var revision orchestrationapi.PersistedRevision; var content string
...
func (s *OrchestrationState) CreateChange(ctx context.Context, requested orchestrationapi.PersistedChange) (orchestrationapi.PersistedChange, bool, error) {
	if requested.Snapshot.ID == "" || requested.IdempotencyKey == "" || requested.Fingerprint == "" || !json.Valid(requested.Input) { return orchestrationapi.PersistedChange{}, false, errors.New("complete change persistence request is required") }
	tx, err := s.db.BeginTx(ctx, nil); if err != nil { return orchestrationapi.PersistedChange{}, false, err }; defer tx.Rollback()
	if _, err := tx.ExecContext(ctx, `SELECT pg_advisory_xact_lock(hashtext($1))`, "change:"+requested.IdempotencyKey); err != nil { return orchestrationapi.PersistedChange{}, false, err }
	...
	if err == nil {
		if existingFingerprint != requested.Fingerprint { return orchestrationapi.PersistedChange{}, false, job.ErrIdempotencyConflict }
		existing, err := loadChange(ctx, tx, existingID); if err != nil { return orchestrationapi.PersistedChange{}, false, err }
		if err := tx.Commit(); err != nil { return orchestrationapi.PersistedChange{}, false, err }
		return existing, false, nil
	}
	if !errors.Is(err, sql.ErrNoRows) { return orchestrationapi.PersistedChange{}, false, err }
	decisionID := "decision-" + requested.Snapshot.ID; d := requested.Snapshot.Decision
	...
	if err != nil { return orchestrationapi.PersistedChange{}, false, err }
...
func (s *OrchestrationState) UpdateChange(ctx context.Context, updated orchestrationapi.PersistedChange) error {
	...
	count, err := result.RowsAffected(); if err != nil { return err }; if count != 1 { return errors.New("change persistence version conflict") }
	for _, approval := range updated.Snapshot.Approvals {
		permissions, err := json.Marshal(approval.Permissions); if err != nil { return err }
...
func loadChange(ctx context.Context, query interface{ QueryRowContext(context.Context,string,...any)*sql.Row }, id string) (orchestrationapi.PersistedChange,error) {
	persisted, err := scanPersistedChange(query.QueryRowContext(ctx, changeSelect+` WHERE c.id=$1`, id)); if err != nil { return persisted, err }
	rows, err := queryApprovals(ctx, query, id); if err != nil { return persisted, err }; persisted.Snapshot.Approvals = rows; return persisted, nil
}

func scanPersistedChange(row scanner) (orchestrationapi.PersistedChange,error) {
	var persisted orchestrationapi.PersistedChange; var input,risk,state,effect,decisionRisk,permissions string
	...
	if err != nil { return persisted, err }
	persisted.Input=json.RawMessage(input); persisted.Snapshot.Risk=policy.Risk(risk); persisted.Snapshot.State=change.State(state); persisted.Snapshot.Decision.Effect=policy.Effect(effect); persisted.Snapshot.Decision.Risk=policy.Risk(decisionRisk); persisted.Snapshot.Decision.Requirement.Permission=permissions
	return persisted,nil
}

func (s *OrchestrationState) loadApprovals(ctx context.Context, changeID string) ([]policy.Approval,error) { return queryApprovals(ctx,s.db,changeID) }
type approvalQuerier interface{ QueryContext(context.Context,string,...any)(*sql.Rows,error) }
func queryApprovals(ctx context.Context, query any, changeID string) ([]policy.Approval,error) {
	querier,ok:=query.(approvalQuerier); if !ok { return nil, errors.New("approval query is not supported") }
	rows,err:=querier.QueryContext(ctx,`SELECT actor,to_json(permissions)::text,approved_at FROM cc_change_approvals WHERE change_id=$1 ORDER BY approved_at,actor`,changeID); if err!=nil{return nil,err};defer rows.Close();approvals:=make([]policy.Approval,0)
	for rows.Next(){var approval policy.Approval;var permissions string;if err:=rows.Scan(&approval.Actor,&permissions,&approval.ApprovedAt);err!=nil{return nil,err};if err:=json.Unmarshal([]byte(permissions),&approval.Permissions);err!=nil{return nil,err};approvals=append(approvals,approval)}
