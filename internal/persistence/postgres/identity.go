...
func (s *IdentityStore) FindSessionByDigest(ctx context.Context, digest string) (auth.Session, error) {
	return scanSession(s.db.QueryRowContext(ctx, `SELECT id::text,user_id::text,token_digest,credential_version,created_at,expires_at,revoked_at,host(source_ip),user_agent FROM cc_auth_sessions WHERE token_digest=$1`, digest))
}
func (s *IdentityStore) FindSessionForUserByID(ctx context.Context, userID, sessionID string) (auth.Session, error) {
	return scanSession(s.db.QueryRowContext(ctx, `SELECT id::text,user_id::text,token_digest,credential_version,created_at,expires_at,revoked_at,host(source_ip),user_agent FROM cc_auth_sessions WHERE user_id=$1::uuid AND id=$2::uuid`, userID, sessionID))
}
func scanSession(row *sql.Row) (auth.Session, error) {
	var session auth.Session
	var revoked sql.NullTime
	var sourceIP sql.NullString
	err := row.Scan(&session.ID, &session.UserID, &session.TokenDigest, &session.CredentialVersion, &session.CreatedAt, &session.ExpiresAt, &revoked, &sourceIP, &session.UserAgent)
	if errors.Is(err, sql.ErrNoRows) {
...
func (s *IdentityStore) ListActiveSessionsForUser(ctx context.Context, userID string, now time.Time) ([]auth.Session, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT id::text,user_id::text,token_digest,credential_version,created_at,expires_at,revoked_at,host(source_ip),user_agent FROM cc_auth_sessions WHERE user_id=$1::uuid AND revoked_at IS NULL AND expires_at>$2 ORDER BY created_at DESC,id ASC`, userID, now.UTC())
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		var revoked sql.NullTime
		var sourceIP sql.NullString
		if err := rows.Scan(&session.ID, &session.UserID, &session.TokenDigest, &session.CredentialVersion, &session.CreatedAt, &session.ExpiresAt, &revoked, &sourceIP, &session.UserAgent); err != nil {
			return nil, err
		}
		if revoked.Valid {
			value := revoked.Time.UTC()
