...
func (s *IdentityStore) FindUserByUsername(ctx context.Context, username string) (auth.User, error) {
	return scanUser(s.db.QueryRowContext(ctx, `SELECT id::text, username, display_name, password_hash, enabled, created_at, last_login_at FROM cc_local_users WHERE lower(username) = lower($1)`, strings.TrimSpace(username)))
}
func (s *IdentityStore) FindUserByID(ctx context.Context, id string) (auth.User, error) {
	return scanUser(s.db.QueryRowContext(ctx, `SELECT id::text, username, display_name, password_hash, enabled, created_at, last_login_at FROM cc_local_users WHERE id = $1::uuid`, id))
}
func scanUser(row *sql.Row) (auth.User, error) {
	var user auth.User
	var lastLogin sql.NullTime
	if err := row.Scan(&user.ID, &user.Username, &user.DisplayName, &user.PasswordHash, &user.Enabled, &user.CreatedAt, &lastLogin); err != nil {
...
func (s *IdentityStore) CreateSession(ctx context.Context, session auth.Session) error {
	_, err := s.db.ExecContext(ctx, `INSERT INTO cc_auth_sessions (id,user_id,token_digest,created_at,expires_at,source_ip,user_agent) VALUES ($1::uuid,$2::uuid,$3,$4,$5,NULLIF($6,'')::inet,$7)`, session.ID, session.UserID, session.TokenDigest, session.CreatedAt.UTC(), session.ExpiresAt.UTC(), session.SourceIP, session.UserAgent)
...
func (s *IdentityStore) FindSessionByDigest(ctx context.Context, digest string) (auth.Session, error) {
	...
	err := s.db.QueryRowContext(ctx, `SELECT id::text,user_id::text,token_digest,created_at,expires_at,revoked_at,host(source_ip),user_agent FROM cc_auth_sessions WHERE token_digest=$1`, digest).Scan(&session.ID, &session.UserID, &session.TokenDigest, &session.CreatedAt, &session.ExpiresAt, &revoked, &sourceIP, &session.UserAgent)
	if errors.Is(err, sql.ErrNoRows) {
...
func BootstrapAdmin(ctx context.Context, db *sql.DB, username, passwordHash string, now time.Time) (string, error) {
	if db == nil || strings.TrimSpace(username) == "" || passwordHash == "" || now.IsZero() {
		return "", errors.New("database, username, password hash, and time are required")
	}
	...
	_, err = tx.ExecContext(ctx, `INSERT INTO cc_local_users (id,username,display_name,password_hash,enabled,created_at,updated_at,password_changed_at) VALUES ($1::uuid,$2,'Control Center Administrator',$3,true,$4,$4,$4) ON CONFLICT DO NOTHING`, userID, strings.TrimSpace(username), passwordHash, now.UTC())
	if err != nil {
		return "", err
	}
	var enabled bool
	if err := tx.QueryRowContext(ctx, `SELECT id::text,enabled FROM cc_local_users WHERE lower(username)=lower($1)`, username).Scan(&userID, &enabled); err != nil {
		return "", err
	}
	if !enabled {
		return "", errors.New("bootstrap administrator exists but is disabled")
	}
