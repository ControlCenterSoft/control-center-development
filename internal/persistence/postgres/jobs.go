...
func (r *JobRepository) Create(ctx context.Context, request job.CreateRequest) (job.Job, bool, error) {
	...
	if err == nil { return created, true, nil }
	if !errors.Is(err, sql.ErrNoRows) {
		if isUniqueViolation(err) { return job.Job{}, false, fmt.Errorf("job id already exists: %s", request.ID) }
		return job.Job{}, false, err
	}
	existing, storedFingerprint, err := r.byIdempotency(ctx, request.IdempotencyKey)
	if err != nil { return job.Job{}, false, err }
	if storedFingerprint != fingerprint { return job.Job{}, false, job.ErrIdempotencyConflict }
...
func (r *JobRepository) Get(ctx context.Context, id string) (job.Job, error) {
	...
	if errors.Is(err, sql.ErrNoRows) { return job.Job{}, job.ErrNotFound }
	return result, err
}
...
func (r *JobRepository) List(ctx context.Context, filter job.Filter) ([]job.Job, error) {
	...
	for rows.Next() { item, err := scanJob(rows); if err != nil { return nil, err }; result = append(result, item) }
	return result, rows.Err()
}

func (r *JobRepository) Claim(ctx context.Context, workerID string, now time.Time, ttl time.Duration) (job.Job, bool, error) {
	if workerID == "" || now.IsZero() || ttl <= 0 { return job.Job{}, false, errors.New("worker id, current time, and positive lease ttl are required") }
	tx, err := r.db.BeginTx(ctx, nil); if err != nil { return job.Job{}, false, err }; defer tx.Rollback()
	_, err = tx.ExecContext(ctx, `UPDATE cc_jobs SET status='cancelled',lease_token=NULL,lease_worker_id=NULL,lease_expires_at=NULL,updated_at=$1,version=version+1 WHERE status='cancel_requested' AND lease_expires_at <= $1`, now.UTC()); if err != nil { return job.Job{}, false, err }
	_, err = tx.ExecContext(ctx, `UPDATE cc_jobs SET status='failed',last_error='maximum attempts exhausted',lease_token=NULL,lease_worker_id=NULL,lease_expires_at=NULL,updated_at=$1,version=version+1 WHERE attempt>=max_attempts AND (status='queued' OR status='retry_wait' OR (status='running' AND lease_expires_at <= $1))`, now.UTC()); if err != nil { return job.Job{}, false, err }
	var id string
	...
	if errors.Is(err, sql.ErrNoRows) { if err := tx.Commit(); err != nil { return job.Job{}, false, err }; return job.Job{}, false, nil }
	if err != nil { return job.Job{}, false, err }
	token := randomHex(32)
...
func (r *JobRepository) RenewLease(ctx context.Context, id, token string, now time.Time, ttl time.Duration) (job.Job, error) {
	...
	if errors.Is(err, sql.ErrNoRows) { return job.Job{}, job.ErrLeaseLost }
	return result, err
}

func (r *JobRepository) Succeed(ctx context.Context, id, token string, output events.Output, now time.Time) (job.Job, error) {
	payload, err := json.Marshal(output); if err != nil { return job.Job{}, err }
	tx, err := r.db.BeginTx(ctx, nil); if err != nil { return job.Job{}, err }; defer tx.Rollback()
	...
	if errors.Is(err, sql.ErrNoRows) { return job.Job{}, job.ErrLeaseLost }; if err != nil { return job.Job{}, err }
	if result.Status == job.StatusSucceeded { if err := persistOutputs(ctx, tx, id, output); err != nil { return job.Job{}, err } }
	if err := tx.Commit(); err != nil { return job.Job{}, err }; return result, nil
}
...
func (r *JobRepository) Fail(ctx context.Context, id, token, message string, output events.Output, retry job.RetryPolicy, now time.Time) (job.Job, error) {
	if message == "" { return job.Job{}, errors.New("failure message is required") }
	payload, err := json.Marshal(output); if err != nil { return job.Job{}, err }
	tx, err := r.db.BeginTx(ctx, nil); if err != nil { return job.Job{}, err }; defer tx.Rollback()
	...
	if errors.Is(err, sql.ErrNoRows) { return job.Job{}, job.ErrLeaseLost }; if err != nil { return job.Job{}, err }
	status := job.StatusFailed; var next any
	if current.Status == job.StatusCancelRequested { status = job.StatusCancelled } else if current.Attempt < current.MaxAttempts { status = job.StatusRetryWait; next = now.UTC().Add(retryDelay(current.Attempt, retry)) }
	...
	if err != nil { return job.Job{}, err }; if err := persistOutputs(ctx, tx, id, output); err != nil { return job.Job{}, err }; if err := tx.Commit(); err != nil { return job.Job{}, err }; return result, nil
}
...
func (r *JobRepository) RequestCancel(ctx context.Context, id string, now time.Time) (job.Job, error) {
	...
	if errors.Is(err, sql.ErrNoRows) { return job.Job{}, job.ErrNotFound }; return result, err
}

type scanner interface{ Scan(...any) error }
func scanJob(row scanner) (job.Job,error) { return scanJobWithExtra(row,nil) }
func scanJobWithExtra(row scanner, extra *string)(job.Job,error){var result job.Job;var input string;var status string;var nextAttempt,leaseExpires sql.NullTime;var leaseToken,leaseWorker,outputJSON,lastError sql.NullString;targets:=[]any{&result.ID,&result.ChangeID,&result.ActionName,&input,&result.IdempotencyKey,&status,&result.Attempt,&result.MaxAttempts,&nextAttempt,&leaseToken,&leaseWorker,&leaseExpires,&outputJSON,&lastError,&result.CreatedAt,&result.UpdatedAt,&result.Version};if extra!=nil{targets=append(targets,extra)};if err:=row.Scan(targets...);err!=nil{return job.Job{},err};result.Input=json.RawMessage(input);result.Status=job.Status(status);if nextAttempt.Valid{result.NextAttemptAt=nextAttempt.Time.UTC()};if leaseToken.Valid&&leaseWorker.Valid&&leaseExpires.Valid{result.Lease=&job.Lease{Token:leaseToken.String,WorkerID:leaseWorker.String,ExpiresAt:leaseExpires.Time.UTC()}};if outputJSON.Valid{var output events.Output;if err:=json.Unmarshal([]byte(outputJSON.String),&output);err!=nil{return job.Job{},err};result.Output=&output};result.LastError=lastError.String;return result,nil}
func persistOutputs(ctx context.Context,tx *sql.Tx,jobID string,output events.Output)error{for _,actual:=range output.ActualStates{_,err:=tx.ExecContext(ctx,`INSERT INTO cc_actual_states (job_id,resource_id,kind,state,observed_at,revision_id,details) VALUES ($1,$2,$3,$4,$5,NULLIF($6,''),NULLIF($7,'')::jsonb) ON CONFLICT (job_id,resource_id) DO UPDATE SET kind=EXCLUDED.kind,state=EXCLUDED.state,observed_at=EXCLUDED.observed_at,revision_id=EXCLUDED.revision_id,details=EXCLUDED.details`,jobID,actual.ResourceID,actual.Kind,string(actual.State),actual.ObservedAt.UTC(),actual.Revision,string(actual.Details));if err!=nil{return err}};for _,health:=range output.Health{_,err:=tx.ExecContext(ctx,`INSERT INTO cc_health_observations (job_id,resource_id,status,checked_at,message) VALUES ($1,$2,$3,$4,NULLIF($5,'')) ON CONFLICT (job_id,resource_id) DO UPDATE SET status=EXCLUDED.status,checked_at=EXCLUDED.checked_at,message=EXCLUDED.message`,jobID,health.ResourceID,string(health.Status),health.CheckedAt.UTC(),health.Message);if err!=nil{return err}};return nil}
func jobFingerprint(request job.CreateRequest)string{h:=sha256.New();h.Write([]byte(request.ChangeID));h.Write([]byte{0});h.Write([]byte(request.ActionName));h.Write([]byte{0});h.Write(request.Input);return hex.EncodeToString(h.Sum(nil))}
func retryDelay(attempt int,retry job.RetryPolicy)time.Duration{base:=retry.BaseDelay;if base<=0{base=time.Second};delay:=base;for i:=1;i<attempt;i++{delay*=2};if retry.MaxDelay>0&&delay>retry.MaxDelay{return retry.MaxDelay};return delay}
func randomHex(size int)string{value:=make([]byte,size);if _,err:=rand.Read(value);err!=nil{panic("operating system random source unavailable")};return hex.EncodeToString(value)}
