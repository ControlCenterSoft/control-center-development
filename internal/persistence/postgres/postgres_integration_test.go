...
func TestPostgresStateSurvivesAdapterRestart(t *testing.T) {
	...
	if err != nil || secondID != userID { t.Fatalf("idempotent bootstrap id=%q second=%q err=%v", userID, secondID, err) }
	identities, _ := NewIdentityStore(db)
	if _, err := identities.FindUserByUsername(ctx, username); err != nil { t.Fatal(err) }
	if !NewAuthorizer(db).Allowed(userID, rbac.PermissionChangesWrite, rbac.GlobalScope()) { t.Fatal("persisted administrator binding did not authorize") }
	...
	if err := identities.CreateSession(ctx, session); err != nil { t.Fatal(err) }
	restartedIdentities, _ := NewIdentityStore(db)
	if restored, err := restartedIdentities.FindSessionByDigest(ctx, session.TokenDigest); err != nil || restored.ID != session.ID { t.Fatalf("session after restart=%#v err=%v", restored, err) }
	auditLog, _ := NewAuditLog(db)
	if err := auditLog.Append(ctx, audit.Event{Action: "integration.restart", Outcome: "success", ActorID: userID}); err != nil { t.Fatal(err) }
	...
	if _, created, err := state.CreateChange(ctx, persisted); err != nil || !created { t.Fatalf("create change created=%v err=%v", created, err) }
	...
	if err := state.UpdateChange(ctx, persisted); err != nil { t.Fatal(err) }
	...
	if _, err := jobs.Succeed(ctx, claimed.ID, claimed.Lease.Token, output, time.Now().UTC()); err != nil { t.Fatal(err) }

	restartedState, _ := NewOrchestrationState(db)
	loaded, err := restartedState.Load(ctx)
	if err != nil { t.Fatal(err) }
	foundChange := false
	for _, candidate := range loaded.Changes { if candidate.Snapshot.ID == changeID { foundChange = candidate.JobID == createdJob.ID && len(candidate.Snapshot.Approvals) == 1 } }
	if !foundChange { t.Fatal("change, approval, or job binding was lost across adapter restart") }
	restartedJobs, _ := NewJobRepository(db)
	if restored, err := restartedJobs.Get(ctx, createdJob.ID); err != nil || restored.Status != job.StatusSucceeded || restored.Output == nil { t.Fatalf("job after restart=%#v err=%v", restored, err) }
	...
	if err != nil { t.Fatalf("startup terminal job reconciliation: %v", err) }
	repaired, err := restartedState.Load(ctx)
	if err != nil { t.Fatal(err) }
	foundRepairedChange := false
	for _, candidate := range repaired.Changes { if candidate.Snapshot.ID == changeID { foundRepairedChange = candidate.Snapshot.State == change.StateSucceeded } }
	if !foundRepairedChange { t.Fatal("startup reconciliation did not repair the terminal job's durable change state") }
}

func hexDigest(value string) string { return hexDigestBytes([]byte(value)) }
func hexDigestBytes(value []byte) string { sum := sha256.Sum256(value); return hex.EncodeToString(sum[:]) }
