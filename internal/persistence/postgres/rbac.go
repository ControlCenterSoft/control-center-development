...
func (a *Authorizer) Allowed(subjectID string, permission rbac.Permission, target rbac.Scope) bool {
	if a==nil||a.db==nil||subjectID==""||permission==""||!target.Valid(){return false}
	ctx,cancel:=context.WithTimeout(context.Background(),2*time.Second);defer cancel();var allowed bool
