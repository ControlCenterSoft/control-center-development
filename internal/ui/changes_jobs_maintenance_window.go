...
// ApplyVerifiedMaintenanceWindowEvidence makes maintenance-window evidence
// visible only after a typed contract has been validated and bound to the exact
// immutable Change revision. Window presence remains non-authorizing.
func ApplyVerifiedMaintenanceWindowEvidence(
	...
	for _, item := range evidence {
		...
		if item.EvaluatedAt.After(now) || item.ObservedAt.After(now) {
