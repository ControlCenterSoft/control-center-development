...
- Последний официально опубликованный исходный релиз: **0.24.0**.
- Отдельный стабильный бинарный канал подтверждён до **0.3.1**.
- Текущий release candidate после 0.24.0: **0.25.0**; до завершения обязательной qualification и официальной публикации он остаётся предварительным.
...
Control Center 0.25.0 готовит permission-gated bounded read-only API чтения событий Audit. Доступ требует аутентифицированной актуальной сессии и соответствующего разрешения; чтение ограничено bounded pagination/filters, возвращает редактированные детали и не добавляет mutation или unbounded export path. Возможность не считается опубликованной до официального релиза.
...
Для чистой установки действует bootstrap-учётная запись `admin` с первоначальным паролем `admin`. При первом входе пароль необходимо сменить; до смены обычная работа запрещена. При обновлении установленный пользователем пароль сохраняется и не сбрасывается к первоначальному значению.
