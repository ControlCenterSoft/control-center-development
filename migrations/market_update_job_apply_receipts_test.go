...
func TestMarketUpdateApplyReceiptMigrationSealsAtomicVerifyingPair(t *testing.T) {
	up := readMarketApplyMigration(t, "0010_market_update_job_apply_receipts.up.sql")
...
func TestMarketUpdateApplyReceiptMigrationPreservesSnapshotAndRollbackSafety(t *testing.T) {
	...
	for _, required := range []string{
		...
		if !strings.Contains(up, required) {
			t.Fatalf("0010 does not preserve apply rollback safety: missing %q", required)
		}
	}

	down := strings.ToUpper(readMarketApplyMigration(t, "0010_market_update_job_apply_receipts.down.sql"))
	for _, required := range []string{
...
func TestMarketUpdateApplyReceiptMigrationIsMarketScoped(t *testing.T) {
	up := strings.ToUpper(readMarketApplyMigration(t, "0010_market_update_job_apply_receipts.up.sql"))
