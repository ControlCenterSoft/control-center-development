...
func TestAuditLookupIndexMigrationIsScopedAndReversible(t *testing.T) {
	up := readMigration(t, "0009_audit_lookup_indexes.up.sql")
	...
	for _, forbidden := range []string{"DROP TABLE", "ALTER TABLE", "DELETE FROM", "TRUNCATE"} {
		if strings.Contains(upper, forbidden) {
			t.Fatalf("0009 lookup migration is not additive: found %q", forbidden)
		}
	}

	down := readMigration(t, "0009_audit_lookup_indexes.down.sql")
	if !strings.Contains(down, "DROP INDEX IF EXISTS cc_audit_events_correlation_sequence_idx") {
		t.Fatal("0009 down migration does not remove only its correlation lookup index")
	}
}
